...
export class RdvService {
  ...
  getAll(datesearch?: Date, idpatient?: number): Observable<Rdv[]> {
    ...
    return this.http.get<Rdv[]>(
      environment.backendUri +
        'rdv?' +
        (datesearch == undefined || ('' + datesearch).length != 10
          ? ''
          : '&datesearch=' + datesearch) +
        (idpatient == undefined || '' + idpatient == '' || idpatient == 0
          ? ''
          : '&patient=' + idpatient),
  ...
  update(rdv: Rdv): Observable<any> {
    return this.http.put(
      environment.backendUri + 'rdv/' + rdv.id,
      rdv,
      this.config.httpOptions
    );
  }
